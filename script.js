...
const featuredProjects = [
  {
    ...
    screenshot: null,
    hasRelease: true,
    readme: { zh: 'doc/README_ZH.md', en: 'doc/README_EN.md' }
  },
  {
...
for (const project of featuredProjects) {
  ...
  const { zh, en } = project.readme;
  if (zh) cachedReadmePathMap[`${project.repo}:zh`] = { path: zh, htmlUrl: null };
  if (en) cachedReadmePathMap[`${project.repo}:en`] = { path: en, htmlUrl: null };
}
...
function renderProjects() {
  ...
  list.innerHTML = featuredProjects.map((project) => {
    ...
    const versionBadge = project.hasRelease
